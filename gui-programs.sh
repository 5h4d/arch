...
echo 'This script will install: wine, lutris, steam, discord, gparted, qbittorrent, spotify, chromium, python, idle, gimp, java, vlc and some gnome extensions'
echo 'Tap enter to continue...'
read tmpvar
sudo sed -i '/^#\[multilib]/{n;s/^#//}' /etc/pacman.conf
sudo sed -i 's/#\[multilib]/\[multilib]/g' /etc/pacman.conf
sudo pacman -Syu
sudo pacman -S lutris steam discord gparted qbittorrent jdk-openjdk chromium gimp pulseaudio pavucontrol python python-pip fish vlc
...
makepkg -si
cd ..
sudo rm -r spotify gnome-shell-extension-ubuntu-dock idle-python3.7-assets gdm-plymouth yay-git ffmpeg-compat-57
